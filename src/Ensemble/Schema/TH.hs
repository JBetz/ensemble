...
deriveCustomFromJSON name = do
  ...
  let body =
        NormalB $
          AppE
            (AppE (VarE 'A.withObject) (LitE $ StringL $ nameBase name))
            ( LamE [VarP objectName] $
                DoE
                  Nothing
  ...
  pure $
    InstanceD
      Nothing
      []
      (AppT (ConT ''A.FromJSON) (ConT name))
  ...
  ConT name ->
    if
      ...
      | otherwise -> do
          ...
          if
            | datatypeVariant typeInfo == Newtype -> showType resolve $ head (constructorFields (head (datatypeCons typeInfo)))
